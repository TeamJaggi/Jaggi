...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if st.get("mode") == "delete_subject":
        if st.get("step") == "choose_exam":
            ...
            await update.message.reply_text(style_text(f"Select subject to DELETE from {txt}:"), reply_markup=subjects_kb(txt))
            return
    ...
    for exam in exams_from_data():
        if txt in subjects_for_exam(exam):
            await update.message.reply_text(style_text(f"{exam} > {txt} – Publishers:"), reply_markup=publishers_kb(exam, txt))
            return
