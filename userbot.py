...
Need assistance? Contact the bot administrator.
    ...
    async def user_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Get user info
        async with self.db.execute(
            "SELECT phone, is_verified, created_at, last_active, has_subscribed FROM users WHERE user_id = ?",
            (user_id,)
        ) as cursor:
            user = await cursor.fetchone()
        ...
        phone, is_verified, created_at, last_active, has_subscribed = user
        ...
        stats_text = (
            ...
            f"📢 Subscribed: {'Yes' if has_subscribed else 'No'}\n"
