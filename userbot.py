...
class ForwardBot:
    ...
    async def load_settings(self):
        async with aiosqlite.connect('auto_forward.db') as conn:
            ...
            result = await cursor.fetchone()
            self.delete_sync = result and result[0] == 'false'
            
            cursor = await conn.execute("SELECT value FROM settings WHERE name='text_only'")
            result = await cursor.fetchone()
            self.text_only = result and result[0] == 'false'
...
async def process_message(event, source_id, target_id, message_text, media=None, reply_to=None):
    ...
    try:
        async with bot.semaphore:
            ...
            if bot.text_only and media:
                ...
                if processed_text:
                    sent_message = await client.send_message(target_id, processed_text, reply_to=reply_to)
                    return sent_message
                return None
            else:
                # Normal mode: forward with media if available
                if media:
                    sent_message = await client.send_file(target_id, media, caption=processed_text, reply_to=reply_to)
                else:
                    sent_message = await client.send_message(target_id, processed_text, reply_to=reply_to)
...
async def handle_new_message(event):
    ...
    try:
        ...
        if event.message.reply_to_msg_id:
            # Check if the replied message exists in our mapping
            replied_msg_id = event.message.reply_to_msg_id
            if source_id in bot.message_mapping and replied_msg_id in bot.message_mapping[source_id]:
                if target_id in bot.message_mapping[source_id][replied_msg_id]:
                    reply_to_id = bot.message_mapping[source_id][replied_msg_id][target_id]
        
        # Create tasks for all target channels
        tasks = []
        for target_id in bot.channel_pairs[source_id]:
            # Get the reply_to ID for this specific target channel
            target_reply_to_id = None
            if event.message.reply_to_msg_id and source_id in bot.message_mapping:
                if event.message.reply_to_msg_id in bot.message_mapping[source_id]:
                    if target_id in bot.message_mapping[source_id][event.message.reply_to_msg_id]:
                        target_reply_to_id = bot.message_mapping[source_id][event.message.reply_to_msg_id][target_id]
            
            task = asyncio.create_task(
                process_message_and_save_mapping(event, source_id, target_id, message_text, media, target_reply_to_id)
            )
            tasks.append(task)
...
async def process_message_and_save_mapping(event, source_id, target_id, message_text, media, reply_to_id=None):
    """Process a message and save the mapping between source and target messages"""
    try:
        sent_message = await process_message(event, source_id, target_id, message_text, media, reply_to_id)
        if sent_message:
