...
# Bot token
TOKEN = "7738808803:AAGWqOIHi8kqmKKeMWWVVLBDk9fQ50H2gZk"
...
# DB setup
async def init_db():
    async with aiosqlite.connect('auto_forward.db', timeout=30) as conn:
        ...
        await conn.execute('''CREATE INDEX IF NOT EXISTS idx_message_mapping_target 
        ...
        await conn.execute("INSERT OR IGNORE INTO settings (name, value) VALUES ('max_concurrent_tasks', '20')")
        await conn.execute("INSERT OR IGNORE INTO settings (name, value) VALUES ('batch_size', '5')")
...
class ForwardBot:
    ...
    async def load_channel_pairs(self):
        async with aiosqlite.connect('auto_forward.db', timeout=10) as conn:
    ...
    async def save_message_mapping_batch(self, mappings):
        """Save multiple message mappings in a single transaction"""
        if not mappings:
            return
            
        try:
            async with aiosqlite.connect('auto_forward.db', timeout=15) as conn:
                for source_message_id, source_channel_id, target_message_id, target_channel_id in mappings:
                    await conn.execute(
                        "INSERT OR REPLACE INTO message_mapping (source_message_id, source_channel_id, target_message_id, target_channel_id) VALUES (?, ?, ?, ?)",
            ...
            logger.error(f"Error saving message mapping batch: {e}")
            return False
        
    async def delete_message_mapping_batch(self, mappings):
        """Delete multiple message mappings in a single transaction"""
        if not mappings:
            return
            
        try:
            async with aiosqlite.connect('auto_forward.db', timeout=15) as conn:
                for source_message_id, source_channel_id, target_channel_id in mappings:
                    if target_channel_id:
                        await conn.execute(
                            "DELETE FROM message_mapping WHERE source_message_id=? AND source_channel_id=? AND target_channel_id=?", 
                            (source_message_id, source_channel_id, target_channel_id)
                        )
                    else:
                        await conn.execute(
                            "DELETE FROM message_mapping WHERE source_message_id=? AND source_channel_id=?", 
                            (source_message_id, source_channel_id)
                        )
                    
                    # Update in-memory mapping
                    if (source_channel_id in self.message_mapping and 
                        source_message_id in self.message_mapping[source_channel_id]):
                        if target_channel_id:
            ...
            logger.error(f"Error deleting message mapping batch: {e}")
                
    async def load_settings(self):
        async with aiosqlite.connect('auto_forward.db', timeout=10) as conn:
            cursor = await conn.execute("SELECT name, value FROM settings")
    ...
    async def is_admin(self, user_id):
        try:
            async with aiosqlite.connect('auto_forward.db', timeout=5) as conn:
    ...
    async def get_edits_and_links(self):
        """Get edits and links with caching for performance"""
        current_time = time.time()
        # Refresh cache every 30 seconds
        if current_time - self.last_cache_refresh > 30:
            try:
                async with aiosqlite.connect('auto_forward.db', timeout=5) as conn:
                    cursor = await conn.execute("SELECT old_text, new_text FROM edits")
                    self.edit_cache = {old: new for old, new in await cursor.fetchall()}
                    
                    cursor = await conn.execute("SELECT old_link, new_link FROM links")
                    self.link_cache = {old: new for old, new in await cursor.fetchall()}
                    
                    self.last_cache_refresh = current_time
            except Exception as e:
                logger.error(f"Error refreshing cache: {e}")
        
        return self.edit_cache, self.link_cache
...
async def reset_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with aiosqlite.connect('auto_forward.db', timeout=30) as conn:
        ...
        await conn.execute("INSERT INTO settings (name, value) VALUES ('max_concurrent_tasks', '20')")
        await conn.execute("INSERT INTO settings (name, value) VALUES ('batch_size', '5')")
...
@admin_required
async def set_batch_size(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        if not context.args:
            await update.message.reply_text(f"Current batch size: {bot.batch_size}")
            return
            
        batch_size = int(context.args[0])
        if batch_size < 1 or batch_size > 20:
            await update.message.reply_text("Please enter a value between 1 and 20")
            return
            
        await bot.set_setting('batch_size', str(batch_size))
        await update.message.reply_text(f"✅ Batch size set to {batch_size}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• /batchsize - Set batch processing size\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "📚 Setup Instructions:\n"
        "1. Add the bot as admin in target channels with necessary permissions\n"
        ...
        "• Use /batchsize to set how many messages to process at once\n\n"
        "📊 Monitoring:\n"
...
async def handle_new_message(event):
    ...
    try:
        ...
        target_ids = bot.channel_pairs[source_id]
        batch_tasks = []
        mapping_batch = []
        
        for i in range(0, len(target_ids), bot.batch_size):
            batch = target_ids[i:i + bot.batch_size]
            
            for target_id in batch:
                task = asyncio.create_task(
                    process_message(event, source_id, target_id, message_text, media)
                )
                batch_tasks.append((target_id, task))
        
        # Wait for batch to complete
        if batch_tasks:
            results = await asyncio.gather(*[task for _, task in batch_tasks], return_exceptions=True)
            
            # Collect mappings for batch save
            for (target_id, _), result in zip(batch_tasks, results):
                if result and hasattr(result, 'id'):
                    mapping_batch.append((event.message.id, source_id, result.id, target_id))
        
        # Save mappings in batch
        if mapping_batch:
            await bot.save_message_mapping_batch(mapping_batch)
...
async def handle_edit(event):
    ...
    try:
        ...
        for target_channel_id, target_message_id in target_messages.items():
            ...
            # Process in batches
            if len(edit_tasks) >= bot.batch_size:
...
async def handle_delete(event):
    ...
    try:
        ...
        for deleted_msg in event.deleted_ids:
            ...
            if source_id in bot.message_mapping and deleted_msg in bot.message_mapping[source_id]:
                ...
                for target_channel_id, target_message_id in target_messages.items():
                    ...
                    delete_mappings.append((deleted_msg, source_id, target_channel_id))
                    
                    # Process in batches
                    if len(delete_tasks) >= bot.batch_size:
        ...
        # Clean up mappings in batch
        if delete_mappings:
            await bot.delete_message_mapping_batch(delete_mappings)
...
async def main():
    ...
    application.add_handler(CommandHandler("batchsize", set_batch_size))
