...
from telethon.tl.types import MessageEntityTextUrl, MessageEntityBold, MessageEntityItalic, MessageEntityCode, MessageEntityPre, MessageEntityMention, MessageEntityHashtag
import asyncio
import logging
...
session_str = "1BVtsOKEBu1MeTv3R34pBdzleykMcaWsuRLdyj2MWrIVEONvC0Lzi8Pr5_LDvNrwHGd4FQSAbDOBeypHfgozOdGcESYzsebxg1DPoVDiB2SEFAuwaQHdf6uReVr2E4vOxtdYBa_q2_rYQ0CJ2pB23MFBahvLPvBQU8a6LrV79MG1jslPNeSyEQB0NCmT2foRuG2Nkykbx-OxhslPaZczfoRJL_o_zqErpC_ByYYolEiWI6dY-0VE6D48iOMse8YjMLtYXl6HZuNkvgnxpuRWb7WXNGSyuu0aaO5CL0PiBoQB8g2r73Tg1C_K1JgaPY12TgTs6oZncAjEeMyXdYULxMOCaceFW3G0="
client = TelegramClient(StringSession(session_str), api_id, api_hash)

# Bot token
TOKEN = "7357029909:AAGrdCKftvwieWcbXoWPS7_Pnu1mkj4ANB8"
...
async def init_db():
    async with aiosqlite.connect('auto_forward.db') as conn:
        ...
        await conn.execute("INSERT OR IGNORE INTO settings (name, value) VALUES ('preserve_formatting', 'true')")
...
class ForwardBot:
    ...
    async def load_settings(self):
        async with aiosqlite.connect('auto_forward.db') as conn:
            ...
            cursor = await conn.execute("SELECT value FROM settings WHERE name='preserve_formatting'")
            result = await cursor.fetchone()
            self.preserve_formatting = result and result[0] == 'true'
    ...
    async def set_setting(self, name, value):
        try:
            ...
            if name == 'forwarding_enabled':
                ...
                self.semaphore = asyncio.Semaphore(self.max_concurrent_tasks)
            elif name == 'preserve_formatting':
                self.preserve_formatting = value == 'true'
        except Exception as e:
...
async def check_settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    formatting_status = "ON ✅" if bot.preserve_formatting else "OFF ❌"
    ...
    if channel_pairs:
        ...
        await update.message.reply_text(
            ...
            f"Preserve Formatting: {formatting_status}\n"
...
async def reset_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with aiosqlite.connect('auto_forward.db') as conn:
        ...
        await conn.execute("INSERT INTO settings (name, value) VALUES ('preserve_formatting', 'true')")
...
@admin_required
async def toggle_formatting(update: Update, context: ContextTypes.DEFAULT_TYPE):
    new_status = not bot.preserve_formatting
    await bot.set_setting('preserve_formatting', 'true' if new_status else 'false')
    
    status_text = "ON ✅" if new_status else "OFF ❌"
    await update.message.reply_text(f"✅ Format Preservation turned {status_text}\n\n"
                                   f"अब messages अपने original formatting के साथ forward होंगे।")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• /formatting - Toggle formatting preservation\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• Use /formatting to preserve text formatting (bold, italic, etc.)\n"
...
async def process_message(event, source_id, target_id, message_text, media=None, formatting_entities=None):
    ...
    try:
        async with bot.semaphore:
            ...
            # Forward the message with preserved formatting if enabled
            if bot.text_only and media:
                # Text-only mode: only forward text
                if processed_text:
                    if bot.preserve_formatting and formatting_entities:
                        # Adjust entity offsets for the processed text
                        adjusted_entities = adjust_entities_for_replacements(
                            formatting_entities, message_text, processed_text, edits, links
                        )
                ...
                if media:
                    if bot.preserve_formatting and formatting_entities and processed_text:
                        # Adjust entity offsets for the processed text
                        adjusted_entities = adjust_entities_for_replacements(
                            formatting_entities, message_text, processed_text, edits, links
                        )
...
def adjust_entities_for_replacements(entities, original_text, processed_text, edits, links):
    """
    Adjust entity offsets after text replacements to maintain proper formatting
    """
    if not entities:
        return entities
        
    # Create a mapping of original text positions to processed text positions
    position_map = create_position_mapping(original_text, processed_text, edits, links)
    
    adjusted_entities = []
    for entity in entities:
        # Adjust the offset based on the position mapping
        adjusted_offset = adjust_position(entity.offset, position_map)
        adjusted_length = adjust_position(entity.offset + entity.length, position_map) - adjusted_offset
        
        if adjusted_offset >= 0 and adjusted_length > 0:
            # Create a new entity with adjusted positions
            if isinstance(entity, MessageEntityTextUrl):
                adjusted_entity = MessageEntityTextUrl(adjusted_offset, adjusted_length, entity.url)
            elif isinstance(entity, MessageEntityBold):
                adjusted_entity = MessageEntityBold(adjusted_offset, adjusted_length)
            elif isinstance(entity, MessageEntityItalic):
                adjusted_entity = MessageEntityItalic(adjusted_offset, adjusted_length)
            elif isinstance(entity, MessageEntityCode):
                adjusted_entity = MessageEntityCode(adjusted_offset, adjusted_length)
            elif isinstance(entity, MessageEntityPre):
                adjusted_entity = MessageEntityPre(adjusted_offset, adjusted_length, entity.language)
            elif isinstance(entity, MessageEntityMention):
                adjusted_entity = MessageEntityMention(adjusted_offset, adjusted_length)
            elif isinstance(entity, MessageEntityHashtag):
                adjusted_entity = MessageEntityHashtag(adjusted_offset, adjusted_length)
            else:
                # For other entity types, just copy with adjusted positions
                adjusted_entity = type(entity)(adjusted_offset, adjusted_length)
                
            adjusted_entities.append(adjusted_entity)
    
    return adjusted_entities

def create_position_mapping(original_text, processed_text, edits, links):
    """
    Create a mapping between positions in the original text and processed text
    """
    # This is a simplified implementation - in a real scenario, you'd need
    # to track how each replacement affects the text length and positions
    # For now, we'll return a basic mapping assuming minimal changes
    mapping = {}
    pos = 0
    
    # For simple cases where text length doesn't change much
    for i in range(max(len(original_text), len(processed_text))):
        if i < len(original_text) and i < len(processed_text):
            mapping[i] = i
        elif i >= len(original_text):
            mapping[i] = len(processed_text)
        else:
            mapping[i] = i
    
    return mapping

def adjust_position(position, position_map):
    """
    Adjust a text position based on the position mapping
    """
    if position in position_map:
        return position_map[position]
    
    # Find the closest position
    closest_pos = min(position_map.keys(), key=lambda x: abs(x - position))
    return position_map[closest_pos]
...
async def handle_new_message(event):
    ...
    try:
        ...
        formatting_entities = event.message.entities if bot.preserve_formatting else None
        
        # Create tasks for all target channels
        tasks = []
        for target_id in bot.channel_pairs[source_id]:
            task = asyncio.create_task(
                process_message_and_save_mapping(event, source_id, target_id, message_text, media, formatting_entities)
            )
            tasks.append(task)
...
async def process_message_and_save_mapping(event, source_id, target_id, message_text, media, formatting_entities):
    """Process a message and save the mapping between source and target messages"""
    try:
        sent_message = await process_message(event, source_id, target_id, message_text, media, formatting_entities)
        if sent_message:
    ...
    try:
        ...
        for target_channel_id, target_message_id in target_messages.items():
            if bot.preserve_formatting and formatting_entities:
                ...
                task = asyncio.create_task(
                    client.edit_message(target_channel_id, target_message_id, processed_text, formatting_entities=adjusted_entities)
                )
            else:
                task = asyncio.create_task(
...
async def main():
    ...
    application.add_handler(CommandHandler("formatting", toggle_formatting))
